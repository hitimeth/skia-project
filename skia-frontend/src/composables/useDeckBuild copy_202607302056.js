...
export function useDeckBuild({
  gridRows = 5,
  gridCols = 5,
  boardCategory = 'nightmare',
  fallbackBoardCategory = null,
  maxTeamMembers = 15,
    ...
    try {
      ...
      for (let i = 0; i < maxSlotCount; i++) {
        ...
        parsedList.push({
          ...
          grid: { row, col },
          position: {
            left: col * cellWidth + (cellWidth - 44) / 2,
            top: row * cellHeight + (cellHeight - 44) / 2
          }
        });
      }
      return parsedList;
  ...
  // === Effect ordering and grouped lists (moved from component to composable) ===
  const MASTER_EFFECT_ORDER = [
    'UTC25','UTC26','STB03','STB07','STB08','STB22','STB01','STB25','SDB10','STB10','STB12','STB24','STB13','STB16','STB21','STB05','STB26','STB19','STB14','SYG01','SYG02'
  ];

  const MASTER_EFFECT_CCCDOT_ORDER = ['DOT01','DOT02','DOT03','DOT04','DOT05','DOT06','CCC01','CCC02','CCC03','CCC04','CCC05','CCC06','CCC07','CCC08','CCC09','CCC10','CCC11'];

  const MASTER_EFFECT_DEBUFF_ORDER = ['SDB17','SDB03','SDB21','SDB20','SDB07','SDB19','SDB08','SDB09','SDB09','SDB06','SDB04','SDB16','SDB18','SDB05','SDB12','SDB22','SDB23','SDB15','SDB14'];
  ...
  const effectGroupedList = computed(() => {
    ...
    MASTER_EFFECT_ORDER.forEach(code => {
      if (!code) return;
      const nameShort = getCodeName(code);
      groups[code] = { effectCode: code, effectNameShort: nameShort, effectName: nameShort, sources: [] };
    });
    ...
    [1,2,3].forEach(teamNo => {
      (teams.value[teamNo] || []).forEach(char => {
        ...
        Object.values(char.buffs).forEach(buff => {
          if (buff && (buff.target_code === 'TGT02' || buff.target_code === 'TGT02')) {
            const code = buff.effect_code || 'NONE';
            if (!groups[code]) {
              const shortName = getCodeName(code);
              groups[code] = { effectCode: code, effectNameShort: buff.effect_code_name_short || shortName || '기타 효과', effectName: buff.effect_code_name || buff.name || shortName || '기타 효과', sources: [] };
            }
    ...
    const result = [];
    MASTER_EFFECT_ORDER.forEach(code => { if (code && groups[code]) { result.push(groups[code]); delete groups[code]; } });
  ...
  const debuffGroupedList = computed(() => {
    const groups = {};
    MASTER_EFFECT_DEBUFF_ORDER.forEach(code => { if (!code) return; const nameShort = getCodeName(code); groups[code] = { effectCode: code, effectNameShort: nameShort, effectName: nameShort, sources: [] }; });
    MASTER_EFFECT_CCCDOT_ORDER.forEach(code => { if (!code) return; const nameShort = getCodeName(code); groups[code] = { effectCode: code, effectNameShort: nameShort, effectName: nameShort, sources: [] }; });
      ...
      (teams.value[teamNo] || []).forEach(char => {
        ...
        Object.values(char.buffs).forEach(buff => {
          if (buff && buff.effect_type === '디버프') {
            ...
            if (!groups[code]) {
              const shortName = getCodeName(code);
              groups[code] = { effectCode: code, effectNameShort: buff.effect_code_name_short || shortName || '기타 디버프', effectName: buff.effect_code_name || buff.name || shortName || '기타 효과', sources: [] };
            }
            let skillLabel = buff.skill_code === 'SKI01' ? 'N' : buff.skill_code === 'SKI02' ? 'C' : 'A';
            groups[code].sources.push({ teamNo, charName: char.name, charValue: buff.effect_value || '0', skillDesc: skillLabel });
          }
  ...
  const createFullHeroCard = (charId, teamNo, row, col) => {
    ...
    const refContainer = arenaRefs.value?.[teamNo];
    const cellHeight = refContainer ? refContainer.getBoundingClientRect().height / gridRows : 44;
